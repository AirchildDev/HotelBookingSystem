...
def booking_page(parent):
    ...
    booking_frame = Frame(parent, bg="#0b132b")
    ...
    def book_room():
        ...
        if check_in >= check_out:
            ...
            logger.warning(
                f"Invalid booking dates: User={session.current_user}, "
        ...
        if not is_room_available(
            ...
            logger.warning(
                f"Booking rejected: User={session.current_user}, "
                f"Room={room}, Check-in={check_in}, "
        ...
        try:
            ...
            logger.info(
                f"Booking created: User={session.current_user}, "
                f"Room={room}, Check-in={check_in}, "
                f"Check-out={check_out}"
            )
            ...
            logger.error(
                f"Booking failed: User={session.current_user}, "
                f"Room={room}, Error={error}"
            )

            messagebox.showerror(
                "Error",
                f"Could not complete booking: {error}"
            )
    ...
    def cancel_booking():
        ...
        try:
            ...
            messagebox.showerror(
                "Error",
                f"Could not cancel booking: {error}"
            )
    ...
    frame = Frame(booking_frame, bg="#0b132b")
    frame.pack(pady=10)
    ...
    combo_room = ttk.Combobox(frame, width=27)
    ...
    btn_frame = Frame(booking_frame, bg="#0b132b")
